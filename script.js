...
const SHAKE_TL = () => timeline({ delay: 0.5 })
  ...
  .to('.cake__candle', {
    ...
    ease: 'Elastic.easeOut', duration: 0.2, stagger: 0.2, scaleY: 1, autoAlpha: 1
  }, 0.2);
...
const LIGHTS_OUT = () => timeline().to('body', {
  ...
  delay: 0.5, '--lightness': 0, duration: 0.1, '--glow-saturation': 0, '--glow-lightness': 0, '--glow-alpha': 1, '--transparency-alpha': 1
});

const RESET = () => {
  // Use autoAlpha to make sure the letters are clickable again when reset
  set('.char', { '--hue': () => Math.random() * 360, '--char-sat': 0, '--char-light': 0, x: 0, y: 0, autoAlpha: 1 });
  ...
  set('.cake__candle', { autoAlpha: 0, scaleY: 0, transformOrigin: '50% 100%' });
};
RESET();
...
// ==========================================================
// THE MOST IMPORTANT FIX: Use autoAlpha to hide the letters
// ==========================================================
.to('.char', { autoAlpha: 0, duration: 0.25 }, '>-0.5') // This makes them invisible AND non-clickable
...
// ==========================================================
// NEW AND IMPROVED EVENT HANDLER FOR ALL DEVICES
// ==========================================================
const handleButtonClick = (e) => {
  // Prevent default behavior, especially important for 'touchend'
  e.preventDefault();
...
// Listen for both touch and click for maximum compatibility
